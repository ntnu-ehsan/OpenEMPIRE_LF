...
def add_lopf_constraints(model, method: str = LOPFMethod.KIRCHHOFF, **kwargs):
    ...
    Kirchhoff-specific kwargs:
        ...
        dc_line_types (list): Transmission-type names to treat as HVDC (controllable links excluded
            from KVL). If omitted, types whose name contains "DC" are auto-detected.
...
def _bind_to_existing_flows(model, flow_dc_like, existing_flow_candidates=()):
    ...
    # map undirected to directed (EMPIRE's FlowDir is indexed (i, j, h, p, w))
    def _fwd(m, i, j, h, w, p):
        return FlowDir[i, j, h, p, w] == flow_dc_like[i, j, h, w, p]
    def _bwd(m, i, j, h, w, p):
        return FlowDir[j, i, h, p, w] == -flow_dc_like[i, j, h, w, p]
...
def _resolve_dc_line_types(m, dc_line_types) -> set:
    """Return the set of transmission-type names to treat as HVDC (controllable links).

    If ``dc_line_types`` is given (recommended), it is used verbatim. Otherwise the types
    are auto-detected from the model's TransmissionType set as any name containing "DC"
    (case-insensitive), e.g. 'HVDC_Cable'. Auto-detection is logged for transparency.
    """
    if dc_line_types:
        return set(dc_line_types)
    auto = set()
    if hasattr(m, "TransmissionType"):
        for t in m.TransmissionType:
            if "DC" in str(t).upper():
                auto.add(t)
    if auto:
        logger.info("LOPF: auto-detected HVDC line types (name contains 'DC'): %s. "
                    "Pass lopf_kwargs['dc_line_types'] to set this explicitly.", sorted(auto))
    return auto


def _dc_corridor_set(m, dc_line_types) -> set:
    """Set of directed (n1,n2) corridors whose line type is HVDC, using EMPIRE's
    TransmissionTypeOfDirectionalLink (n1,n2,type). Both orientations are included."""
    dc_types = _resolve_dc_line_types(m, dc_line_types)
    dc = set()
    if dc_types and hasattr(m, "TransmissionTypeOfDirectionalLink"):
        for (n1, n2, t) in m.TransmissionTypeOfDirectionalLink:
            if t in dc_types:
                dc.add((n1, n2))
                dc.add((n2, n1))
    return dc
...
def _add_kirchhoff_constraints(
    ...
    *,
    ...
    existing_flow_candidates: tuple[str, ...] = (
        "transmisionOperational",    # EMPIRE's actual (mis-spelled) flow variable name
        "transmissionOperational",   # corrected spelling, if present
        "transFlow",
        "lineFlow",
        "flow",
    ),
    ...
    **_ignored,  # absorb reader-only kwargs (e.g. reactance_per_km)
    ...
    # Resolve the reactance by *name* (not by capturing the abstract component): the rules below
    # must look it up on the instance `m` at construction time, otherwise they would reference the
    # never-constructed AbstractModel component.
    if hasattr(model, reactance_param_name):
        x_name = reactance_param_name
        ...
        def _x_init(m, i, j):
            bij = float(getattr(m, susceptance_param_name)[i, j])
            if abs(bij) < eps:
                return 1.0 / eps
            return 1.0 / bij
    ...
    # HVDC corridors are converter-controlled (they do NOT obey KVL), so they are
    # excluded from the cycle basis: they act as free, controllable links bounded
    # only by the net-flow binding and thermal capacity below. AC lines stay in KVL.
    model._lopf_edge_signs = {}  # {cycle_idx: {(i,j): -1/0/+1}}

    def _cycles_index_init(m):
        dc_corridors = _dc_corridor_set(m, dc_line_types)
        ac_arcs = [(i, j) for (i, j) in m.BidirectionalArc
                   if (i, j) not in dc_corridors and (j, i) not in dc_corridors]
        if dc_corridors:
            logger.info("LOPF: %d AC corridors in KVL cycle basis; %d HVDC corridors excluded (treated as controllable links).",
                        len(ac_arcs), len(m.BidirectionalArc) - len(ac_arcs))
        cycles, edge_signs = _fundamental_cycles(m.Node, ac_arcs)
    ...
    if couple_to_existing_flows:
        ...
        def _net_bind(m, i, j, h, w, p):
            ...
            # Resolve the flow variable on the instance `m` (not the abstract model).
            # EMPIRE's directed flow variable transmisionOperational is indexed
            # (link, Operationalhour, PeriodActive, Scenario) -> (i, j, h, p, w).
            FlowDir = getattr(m, flow_name)
            return FlowDir[i, j, h, p, w] - FlowDir[j, i, h, p, w] == m.FlowK[i, j, h, w, p]
...
def _fundamental_cycles(NodeSet, LineSet) -> Tuple[List[List[Tuple]], Dict[int, Dict[Tuple, int]]]:
    """Build a fundamental cycle basis via spanning tree + chords.

    Returns (cycles, edge_signs) where edge_signs is nested and keyed by the **1-based**
    cycle index used by the Cycle set: {cycle_index: {(i, j): -1/+1}}.
    """
    ...
    cycles = []
    edge_signs: Dict[int, Dict[Tuple, int]] = {}
    for chord in chords:
        u, v = chord
        pth = path_edges(u, v)
        cyc_edges = pth + [chord]
        cycles.append(cyc_edges)
        c1 = len(cycles)  # 1-based cycle index, matching the Cycle set
        signs = edge_signs.setdefault(c1, {})
        ...
        signs[chord] = -1  # closing chord traversed v->u (opposite stored (u,v))
