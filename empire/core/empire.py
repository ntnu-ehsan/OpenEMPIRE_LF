...
def run_empire(name, tab_file_path: Path, result_file_path: Path, scenario_data_path,
    ...
    if IAMC_PRINT:
        ...
        for w in instance.Scenario:
            ...
            for g in instance.Generator:
                f = row_write(f, "Europe", "Active Power|Electricity|"+dict_generators[str(g).strip()], "MWh", "Year", \
        ...
        for g in instance.Generator:
            ...
            if value(instance.genMargCost[g,instance.PeriodActive[1]]) != 0: 
                f = row_write(f, "Europe", "Variable Cost|Electricity|"+dict_generators[str(g).strip()], "EUR/MWh", "Year", [value(instance.genMargCost[g,i]) for i in instance.PeriodActive])
            f = row_write(f, "Europe", "Investment|Energy Supply|Electricity|"+dict_generators[str(g).strip()], "billion US$2010/yr", "Year", [value((1/instance.LeapYearsInvestment)*USD10perEUR18* \
